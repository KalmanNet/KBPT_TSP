...
class KalmanFilter:
    ...
    def __init__(self, SystemModel):
        ...
        self.T_train = SystemModel.T_train;
        self.T_test = SystemModel.T_test;
        self.T_cv  = SystemModel.T_cv

        self.is_vanila_PCI=False
    ...
    def GenerateSequence(self, y, T):
        ...
        self.dy_vector =  torch.empty(size=[self.n, T])
        self.y_vector = torch.empty(size=[self.n, T])
        self.m2y_vector = torch.empty(size=[self.n, T])
        ...
        for t in range(0, T):
            ...
            self.dy_vector[:, t] = self.dy.squeeze()
            self.y_vector[:, t] = self.m1y.squeeze()
            if self.is_vanila_PCI==False:
                self.m2y_vector[:, t] = self.m2y.squeeze()